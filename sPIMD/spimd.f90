...
implicit none
    ...
    write(ana_unit,*) 'nstep    ','aver_x    ','x1   ','pe   ','ke    ', 'kev   ','tote    ','is_eqb  '
    !!
...
implicit none
    ...
    if (md_erst .ne. 1 .or. md_npass > md_nstep/2 ) then
        write (ana_unit,*) md_npass, sum(bd_x)/real(md_bead), bd_x(1), md_pe, md_ke, md_kev, md_tote, md_is_eqb
    end if
    do i=1,md_bead
    ...
    do i=1,md_bead
        select case (md_thermo)
            case (0) !------------------------ fix bug
                ...
                if(i==1) then
                	bd_p(i) = tmplst( 2*n_dvd + 1 , i ) * bd_p(i) + tmplst( 2*n_dvd + 2, i ) * et
                else
                	bd_p(i) = tmplst( 2*n_dvd + 1 , i ) * bd_p(i) + sqrt(real(i)/real(i-1))*tmplst( 2*n_dvd + 2, i ) * et
                end if
            case (1)
...
implicit none
    ...
    !!
    do i=1,md_bead
        bd_ks(i) = bd_ks(i) + bd_p(i) * md_dtime / ( real(n_dvd) * bd_m(i) )
    end do
    call calc_x()
    call calc_fks()
    call calc_pe()
