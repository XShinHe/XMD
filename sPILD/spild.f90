!! A simplest version of pimd of i-dimension (of harmonic oscillator potential)
!!
! ------------------------- basic module -------------------------
!!
module md_spimd
implicit none
    ...
    !!
    integer :: md_bead
    real(kind=8) :: md_bfreq
    real(kind=8) :: md_bf2
    real(kind=8) :: md_masstherm
    real(kind=8) :: md_gammaAD
    !!
    integer :: md_opt = 0
...
implicit none
    integer :: i
    mda_x2 = bd_x(1)**2 + 0.25*md_beta/md_masstherm - 0.25*(bd_p(1)*md_beta/md_masstherm)**2
    mda_p = bd_m(1)/md_masstherm*bd_p(1)
...
implicit none
    integer :: i
    md_pe = 0.0
    md_ke = 0.5 * md_bead * md_temp
    md_kev = 0.0
    do i=1,md_bead
        md_pe = md_pe + fun_pes(bd_x(i))
        md_kev = md_kev + 0.5 * bd_x(i) * bd_fx(i)
        if(i.eq.1) cycle
        md_ke = md_ke - 0.5 * md_mass * md_bf2 * bd_ks(i)*bd_ks(i)*real(i)/real(i-1)
    end do
    md_pe = md_pe / real(md_bead)
    md_kev = md_kev/real(md_bead)
    md_tote = md_pe + md_ke
...
implicit none
    ...
    character(len=32) :: tmpin1, tmpin2
    real(kind=8) :: tmpin3
    integer :: istat, n_init
    logical :: alive
    !!
    n_init = 0
    inquire(file='put.rc', exist=alive)
    if (alive .eqv. .true.) then
        ...
        print *, 'file ./put.rc is not existing, please check it'
    end if
    !!
    if (md_opt .eq. 1) then
    ...
    !!
    md_masstherm = md_beta * 2._8* md_mass * 0.25_8/dtanh(0.5_8*md_beta)
    open(unit=10,file='.mth.tmp',status='old')
        read(10,*) tmp1
        md_masstherm = md_beta * 2*md_mass * tmp1
    close(unit=10)
    print *, 'massthermo is ', md_masstherm
    !!
    md_npass = 0
    md_is_eqb = 0
...
implicit none
    ...
    write(ana_unit,*) 'nstep    ','x1    ','p1   ','p-beta    ', 'pe   ','pe-beta    ','pe-pimd  '
    !!
...
implicit none
    ...
    if (md_erst .ne. 1 .or. md_npass > md_nstep/2 ) then
        write (ana_unit,*) md_npass, bd_x(1), bd_p(1), mda_p, 0.5_8*bd_x(1)**2, 0.5_8*mda_x2, md_pe
    end if
    do i=1,md_bead
        !write (out_unit,*) md_npass, i, bd_ks(i), bd_p(i), bd_x(i), bd_fks(i), bd_fx(i)
    end do
    ...
    do i=2,md_bead  !-------------------------------------------------------------------------------------
        select case (md_thermo)
            case (0)
                ...
                bd_p(i) = tmplst( 2*n_dvd + 1 , i ) * bd_p(i) + dsqrt(md_gammaAD*real(i)/real(i-1)) & ! fix bugs
                	*tmplst( 2*n_dvd + 2, i ) * et
            case (1)
                call random_number(md_rand)
                if ( md_rand < 1.- tmplst( 2*n_dvd + 1 , i ) ) then
                    call random_norm(et, md_rand, 1.0_8)
                    bd_p(i) = et * tmplst(1,i)
                end if
            case (2)
...
implicit none
    ...
    integer :: i
    !!
    bd_p(1) = bd_p(1) - (md_masstherm/bd_m(1)) * bd_fks(1) * md_dtime / real(n_dvd) !----------------------
    do i=2,md_bead
