...
implicit none
    ...
    integer :: i,j
    !!
    real(dp), dimension(3) :: geom
    real(dp), dimension(3,3) :: cart
    geom=(/1.81_dp,1.81_dp,1.8239_dp/)
    !!
    !! if use NHC, initialize NHC settings
    if (md_thermo.eq.2) then
        call generate_NHC(mobj%nb, 3, 5) 
    end if
    !! use transform method, initialize relative settings
    ...
    !!
    ! mod==1, for equilibrium mode
    if(md_mod.eq.1) then
    	! z-coordinates to Cartesian
        ...
        ! Cartesian to staging coordinates
        do i=1,mobj%nb
        ...
        ! need force for first step propagation
        call md_update(mobj)
    end if
