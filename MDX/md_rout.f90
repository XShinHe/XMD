...
contains
	...
	implicit none
		...
		!------  for md_ipimd = 1 : calling staging transform settings
		!------  for md_ipimd = 2 : calling normal-mode transform settings
		...
		!------ 1) md_ischeme = 1: middle schem
		!------ 2) md_ischeme = 2: end schene
		!------ 3) md_ischeme = 3: side scheme
	...
	!-- algorithms:
	!------ 1) md_ithermo = 0 : Langevin thermostat
	!------ 2) md_ithermo = 1 : Andersen thermostat
	!------ 3) md_ithermo = 2 : Nose-Hoover chain thermostat, (see thermo_plus.f90 for more details)
	...
	implicit none
		...
		integer :: i,j,k
		real(dp) :: scalar
		real(dp), dimension(md_dim) :: r_c
		
		mobj%a(i)%pe = 0.0_dp
		mobj%a(i)%ke = 0.5_dp * md_dim * md_nsum * md_temp
		!-- for original estimitor as:
		...
		do i=1,md_nsum
			do k=i+1,md_nsum
				do j=1,md_bead
				    ...
				    mobj%a(i)%ke = mobj%a(i)%ke + 0.5_dp/real(md_bead) * ( mobj%a(i)%x(md_dim*j-2) * mobj%a(i)%fx(md_dim*j-2) + &
				            mobj%a(i)%x(md_dim*j-1) * mobj%a(i)%fx(md_dim*j-1) + mobj%a(i)%x(md_dim*j) * mobj%a(i)%fx(md_dim*j) )
				    if(j.eq.1) cycle
				    !-- for original estimitor
